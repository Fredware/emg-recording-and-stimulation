...
for i=1:fig.n_rec
    addpoints(fig.line_handles{i}, timestamp, max( emg_buffer.data(emg_buffer.ptr_prev:emg_buffer.ptr-1, i)));
    addpoints(fig.line_handles{i}, timestamp, min( emg_buffer.data(emg_buffer.ptr_prev:emg_buffer.ptr-1, i)));
end
...
if timestamp > fig.t_max
    ...
    if ctrl_thresh > 0
        for i=1:fig.n_ctrl
            subplot(fig.n_rec+i)
            hold on;
            plot([fig.t_min, fig.t_max], [ctrl_thresh, ctrl_thresh], 'r');
        end
    end
end
